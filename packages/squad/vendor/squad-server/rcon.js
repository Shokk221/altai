...
export default class SquadRcon extends Rcon {
  processChatPacket(decodedPacket) {
    ...
    // Debug: chat paketi geldi ama regex eşleşmedi
    if (decodedPacket.body && decodedPacket.body.includes('Chat')) {
      console.warn(`[DEBUG-RCON] Chat packet received but REGEX DID NOT MATCH: ${decodedPacket.body.substring(0, 200)}`);
    }

    const matchPossessedAdminCam = decodedPacket.body.match(
      /\[Online I[Dd]s:([^\]]+)\] (.+) has possessed admin camera\./
    );
    ...
    const matchUnpossessedAdminCam = decodedPacket.body.match(
      /\[Online I[Dd]s:([^\]]+)\] (.+) has unpossessed admin camera\./
    );
  ...
  async getCurrentMap() {
    ...
    if (!match) return { level: null, layer: null };
  ...
  async getListPlayers() {
    ...
    for (const line of response.split('\n')) {
      const match = line.match(
        /^ID: (?<playerID>\d+) \| Online IDs:([^|]+)\| Name: (?<name>.+) \| Team ID: (?<teamID>\d|N\/A) \| Squad ID: (?<squadID>\d+|N\/A) \| Is Leader: (?<isLeader>True|False) \| Role: (?<role>[A-Za-z_0-9]+).*$/
      ); // 'role' patched: sometimes when server is under heavy load, role has ',' at end
      if (!match) continue;
