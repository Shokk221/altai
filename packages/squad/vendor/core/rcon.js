...
export default class Rcon extends EventEmitter {
  constructor(options = {}) {
    ...
    this.client.setMaxListeners(50); // Allow many concurrent RCON commands
    ...
    this.count = 1;
    this.loggedin = false;
    this.pendingErrorHandlers = []; // Track write() error handlers for cleanup
  }
  ...
  onClose(hadError) {
    ...
    // Clean up orphaned error listeners from pending write() calls
    if (this.pendingErrorHandlers.length > 0) {
      Logger.verbose('RCON', 2, `Clearing ${this.pendingErrorHandlers.length} pending error handlers`);
      for (const handler of this.pendingErrorHandlers) {
        this.client.removeListener('error', handler);
      }
  ...
  write(type, body) {
    return new Promise((resolve, reject) => {
      ...
      const onError = (err) => {
        ...
        // Remove this handler from the pending list
        this.pendingErrorHandlers = this.pendingErrorHandlers.filter(h => h !== onError);
        // Clean up all other pending error handlers since we're wiping the queue
        for (const handler of this.pendingErrorHandlers) {
          this.client.removeListener('error', handler);
        }
        this.pendingErrorHandlers = [];
      ...
      this.pendingErrorHandlers.push(onError);
      this.client.once('error', onError);
